...
uint8_t track3_flag = 0;          //0->循迹   1->直走
uint8_t track3_turn_flag = -1;     //转向方向
uint8_t track3_end_flag = 0;      //结束标志位
...
void Track3_Start()
{
	...
	if(previouscur_track_state != cur_track_state)
    {
		...
		track3_end_flag ++;
		Start_Angle_Turn(track3_turn_flag * TRACK3_TURN_ANGLE);
		Distance_Init();
    }
